...
export const downloadTcx = (datapoints: DataPoint[], distance: number) => {
  const startTime = datapoints[0].timestamp;

  const output = toTcxString(datapoints, distance);
        ...
        [
          ...
          data.position !== undefined
            ? [
                ...
                `            </Position>`,
                `            <DistanceMeters>${data.position.deltaDistance}</DistanceMeters>`,
              ].join('\n')
            : '',
